class Workbench {
  ...
  public stack(
    analytics: Array<Analytic> = new Array<Analytic>(),
    maxSeconds = 0
  ): Array<Analytic> {
    ...
    analytics.forEach((analytic) => {
      if (timestampStart == null) {
        ...
        if (analytic.getTimestamp() - timestampStart >= maxSeconds) {
          ...
          if (
            itemBeingWorkedOn.getOriginal() instanceof VoteAnalytic ||
            ...
            fields.forEach((field) => {
              analytic.getFields().forEach((fieldTemporal) => {
                if (
                  field.getTechnicalName() == fieldTemporal.getTechnicalName()
                ) {
                  var newField = new AnalyticField(
                    field.value + fieldTemporal.value,
                    field.name,
                    field.technicalName
                  );
            ...
            itemBeingWorkedOn.getOriginal() instanceof GrowthAnalytic
          ) {
            var fields = itemBeingWorkedOn.getFields();
            var newFields = new Array<AnalyticField>();
            fields.forEach((field) => {
